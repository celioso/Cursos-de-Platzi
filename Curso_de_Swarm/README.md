...
Iniciar contenedor `docker run --name aminweb --rm -it -p 8080:80 aminespinoza/linktree:latest`

para iniciar el contenedor pero con la terminal de comandos con bash `docker run --name aminweb --rm -it -p 8080:80 aminespinoza/linktree:latest /bin/bash`

para iniciar el contenedor pero con la terminal de comandos con shell `docker run --name aminweb --rm -it -p 8080:80 aminespinoza/linktree:latest sh`

para inspecciona rel contenedor `docker inspect <CONTEINER ID>`

## Guardar y recuperar imágenes de Docker

Para importar la imagen a un archivo comprimido `docker save -o linktree.rar aminespinoza/linktree:latest` lo primero es el nombre de la imagen y después del mayor que como desea llamarlo y la extensión 

para cargar la imagen en otro docker `docker load -i linktree.rar` y ver si fue exitosa la carga se utiliza `docker images`
...
crear imagen `docker compose build` luego la se crea el contenedor `docker compose up` he ingresamos al siguiente link `http://localhost:8080/linktree.html`