...
#Función asíncrona para procesae el pago
...
#Función intensivo en CPU para calcular el costo total del pedido
...
async def process_order(order_id, items):
    print(f'Iniciando el procesamiento dde la orden {order_id}...')
    ...
    if not all(inventory_results):
        print(f'Order {order_id} cancelada: Producto no está disponible')
    ...
    if payment_result:
        print(f'Order {order_id} completada con éxito. Total: {total}')
